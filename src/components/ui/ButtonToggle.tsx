...
const ButtonToggle: React.FC<Props> = ({ options, onChange, defaultValue }) => {
  ...
  const getButtonClass = (coin: string) =>
    `h-full font-normal px-3 rounded-md transition-all duration-200 cursor-pointer text-[14px] w-full md:w-fit ${
      active === coin
  ...
  return (
    <div className="bg-[#0F0F0F] h-full border border-[#181818] p-1 rounded-[8px] flex gap-2 w-full md:w-fit">
      {options.map((coin) => (
        <button
          key={coin}
